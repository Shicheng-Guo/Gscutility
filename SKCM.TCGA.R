###   uthor: Shicheng Guo, Ph.D. Email: Shicheng.Guo@hotmail.com 
###   Time :  Sep/14/2015 
###   New: geom_errbar in case and control were seperated (complete tonight)
###   Usage: change skcm to skcm, lihc, prad, gbm, brca, blic and so on to analysis DMR, DGE and its regulation effect.
#######################################################################################################################
...
#######################################################################################################################
#######################################  ESCA DNA methylation Collection ##############################################
#######################################################################################################################
# primary directory
priDir="/home/sguo/tcga/skcm/"                                                        # main directory
methDir=paste(priDir,"DNA_Methylation/JHU_USC__HumanMethylation450/Level_3/",sep="")  # Methylation Dataset
methDataSave1="TCGA.Methy450.skcm.RData"                                              # save methylation RData
methDataSave2="TCGA.Methy450.skcm.RM.Impute.RData"                                    # save imputate methylation RData
setwd(methDir)
...
#######################################################################################################################
#######################################  skcm DNA methylation Analysis ##############################################
#######################################################################################################################
priDir="/home/sguo/tcga/skcm/"
methDataSave2="TCGA.Methy450.skcm.RM.Impute.RData"
setwd(priDir)
load(methDataSave2)
library("ChAMP")
...
write.table(rlt1,file="TCGA.Meth450.skcm.ChAMP.DMS.txt",col.names=T,row.names=F,quote=F,sep="\t")
write.table(rlt2$dmrs,file="TCGA.Meth450.skcm.ChAMP.DMR.txt",col.names=T,row.names=F,quote=F,sep="\t")
write.table(rlt2$dmr.probes,file="TCGA.Meth450.skcm.ChAMP.DMR.Probe.txt",col.names=T,row.names=F,quote=F,sep="\t")

#######################################################################################################################
#######################################  ESCA Gene expression analysis ##############################################
#######################################################################################################################

setwd("/home/sguo/tcga/skcm/RNASeqV2/UNC__IlluminaHiSeq_RNASeqV2/Level_3")
file=list.files(pattern="*rsem.genes.normalized_results")
sampleinfo=read.table("/home/sguo/tcga/skcm/file_manifest.txt",head=T,sep="\t")
expDataSave1="/home/sguo/tcga/skcm/TCGA.RNAseqV2.skcm.RData"    # save RNAseqV2 matrix
exptxtSave1="/home/sguo/tcga/skcm/TCGA.RNAseqV2.skcm.Diff.txt"  # save signficant differential gene ouput
...
######################################################################################################################
setwd("/home/sguo/tcga/skcm/")
methy.data<-read.table("TCGA.Meth450.skcm.ChAMP.DMR.txt",head=T,sep="\t",as.is=T)
exp.data<-read.table("TCGA.RNAseqV2.skcm.Diff.txt",head=T,sep="\t",as.is=T)
...
# annotation 
mrg<-read.table("/home/sguo/annotation/MethylationRegulatedGenePanCancer.txt",head=T,as.is=T,sep="\t")
srg<-read.table("/home/sguo/annotation/SmokingRelatedGeneMethylation.bed",head=F,as.is=T,sep="\t")
mig<-c("AHRR")
output1<-subset(output,abs(Delta.beta)>0.1)
output2<-output[na.omit(match(mrg[,1],output$Gene.Symbol)),]
output3<-output[na.omit(match(mig,output$Gene.Symbol)),]
output4<-output[na.omit(match(srg[,5],output$Gene.Symbol)),]
dim(output1)
dim(output2)
dim(output3)
dim(output4)

write.table(output1,file="TCGA.skcm.Methylation.Expression.Significant.txt",col.names=T,row.names=F,sep="\t",quote=F)
write.table(output2,file="TCGA.skcm.Methylation.Expression.sig.MethyRegGene.txt",col.names=T,row.names=F,sep="\t",quote=F)
write.table(output3,file="TCGA.skcm.Methylation.Expression.Sig.MyInterestGene.txt",col.names=T,row.names=F,sep="\t",quote=F)
write.table(output4,file="TCGA.skcm.Methylation.Expression.Sig.SmokingResponse.txt",col.names=T,row.names=F,sep="\t",quote=F)
...
setwd("/home/sguo/tcga/skcm/")
load("TCGA.Methy450.skcm.RM.Impute.RData")
library("ggplot2")
map<-methdata$map
data<-methdata$matrix

# situation 1, plot all the DMR genes
DMR<-read.table("TCGA.Meth450.skcm.ChAMP.DMR.txt",head=T,sep="\t",as.is=T)
DMR<-subset(DMR,abs(deltaBeta)>0.1)
dmr.gene<-unique(DMR$gene)

# situation 2, plot some candidate DMR genes
target.file<-read.table("TCGA.skcm.Methylation.Expression.Significant.txt",head=T,sep="\t",as.is=T)
dmr.gene<-unique(target.file$Gene.Symbol)

# situation 3, plot some candidate DMR genes
...
for(i in 1:length(dmr.gene)){  # length(dmr.gene)
  ...
  if(nrow(map.tmp)>2){
    ...
    limit1min<-mean1-1.96*sd1/sqrt(length(x1))
    limit1max<-mean1+1.96*sd1/sqrt(length(x1))
    limit2min<-mean2-1.96*sd2/sqrt(length(x2))
    limit2max<-mean2+1.96*sd2/sqrt(length(x2))
    tmp<-rbind(data.frame(cpg=names(mean1),mean=mean1,sd=sd1,lim.up=limit1max,lim.down=limit1min,type="Cancer"),
               data.frame(cpg=names(mean2),mean=mean2,sd=sd2,lim.up=limit2max,lim.down=limit2min,type="Normal"))
    ...
    qplotrlt<-qplot(cpg, mean, colour=type, data=tmp) + 
      ...
      geom_errorbar(aes(ymin =lim.down, ymax =lim.up))+
...
# situation 2 plot some candidate DGE genes
target.file<-read.table("TCGA.skcm.Methylation.Expression.Significant.txt",head=T,sep="\t",as.is=T)
