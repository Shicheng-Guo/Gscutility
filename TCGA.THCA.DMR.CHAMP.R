#######################################################################################################################
###   Title : DMR and DVR analysis to thca dataset in TCGA Project with CHAMP packages
###   Author: Shicheng Guo, Ph.D. Email: Shicheng.Guo@hotmail.com 
###   Time :  Sep/9/2015 
###   Section 1.  Download DNA methylation and RNA-seqV2 dataset, clinical information of thca and thca
...
#######################################################################################################################
# primary directory
priDir="/home/sguo/tcga/thca/"                                                        # main directory
methDir=paste(priDir,"DNA_Methylation/JHU_USC__HumanMethylation450/Level_3/",sep="")  # Methylation Dataset
methDataSave1="TCGA.Methy450.thca.RData"                                              # save methylation RData
methDataSave2="TCGA.Methy450.thca.RM.Impute.RData"                                    # save imputate methylation RData
setwd(methDir)
...
#######################################################################################################################
#######################################  thca DNA methylation Analysis ##############################################
#######################################################################################################################
priDir="/home/sguo/tcga/thca/"
methDataSave2="TCGA.Methy450.thca.RM.Impute.RData"
setwd(priDir)
load(methDataSave2)
library("ChAMP")
...
write.table(rlt1,file="TCGA.Meth450.thca.ChAMP.DMS.txt",col.names=T,row.names=F,quote=F,sep="\t")
write.table(rlt2$dmrs,file="TCGA.Meth450.thca.ChAMP.DMR.txt",col.names=T,row.names=F,quote=F,sep="\t")
write.table(rlt2$dmr.probes,file="TCGA.Meth450.thca.ChAMP.DMR.Probe.txt",col.names=T,row.names=F,quote=F,sep="\t")

#######################################################################################################################
#######################################  ESCA Gene expression analysis ##############################################
#######################################################################################################################

setwd("/home/sguo/tcga/thca/RNASeqV2/UNC__IlluminaHiSeq_RNASeqV2/Level_3")
file=list.files(pattern="*rsem.genes.normalized_results")
sampleinfo=read.table("/home/sguo/tcga/thca/file_manifest.txt",head=T,sep="\t")
expDataSave1="/home/sguo/tcga/thca/TCGA.RNAseqV2.thca.RData"    # save RNAseqV2 matrix
exptxtSave1="/home/sguo/tcga/thca/TCGA.RNAseqV2.thca.Diff.txt"  # save signficant differential gene ouput
...
######################################################################################################################
setwd(priDir)
methy.data<-read.table("TCGA.Meth450.thca.ChAMP.DMR.txt",head=T,sep="\t",as.is=T)
exp.data<-read.table("TCGA.RNAseqV2.thca.Diff.txt",head=T,sep="\t",as.is=T)
...
dim(output)
head(output)
subset<-subset(output,abs(Delta.beta)>0.1)
write.table(subset,file="TCGA.thca.Methylation.Expression.Significant.txt",col.names=T,row.names=F,sep="\t",quote=F)
...
load("TCGA.Methy450.thca.RM.Impute.RData")
library("ggplot2")
map<-methdata$map
data<-methdata$matrix

# situation 1, plot all the DMR genes
DMR<-read.table("TCGA.Meth450.thca.ChAMP.DMR.txt",head=T,sep="\t",as.is=T)
DMR<-subset(DMR,abs(deltaBeta)>0.1)
dmr.gene<-unique(DMR$gene)

# situation 1, plot some candidate DMR genes
target.file<-read.table("TCGA.thca.Methylation.Expression.Significant.txt",head=T,sep="\t",as.is=T)
...
for(i in 1:length(dmr.gene)){  # length(dmr.gene)
  ...
  if(nrow(map.tmp)>2){
    ...
    qplotrlt<-qplot(cpg, mean, colour=type, data=tmp) + 
      ...
      geom_errorbar(aes(ymin = mean - 1.96*sd/sqrt(length(x1)+length(x2)), ymax = mean + 1.96*sd/sqrt(length(x1)+length(x2))))+
